...
import { Edit, Star, Trash2, UploadCloud } from 'lucide-react';
...
export default function AdminUserProfilePage() {
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.secure_url) {
        setFormValues((prev) => ({ ...prev, avatar: data.secure_url }));
  ...
  const deleteOrder = async (index) => {
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/admin/user/${id}/orders/${index}`, {
      method: 'DELETE',
  ...
  const deleteReview = async (index) => {
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/admin/user/${id}/reviews/${index}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await res.json();
      ...
      <div className="max-w-3xl mx-auto py-10 px-4 md:px-8">
        <div className="bg-background border border-orange-400 dark:bg-gray-900 shadow-lg rounded-2xl p-8 space-y-8">
          <div className="text-center">
            <h1 className="text-3xl font-bold text-orange-400">Edit User</h1>
            <p className="text-muted-foreground">
              Managing user: <span className="text-orange-400 font-medium">{user.name}</span>
            </p>
          </div>
          ...
          <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            ...
            <p className="font-semibold select-text">{formValues.email}</p>
          </div>

          {/* Orders */}
          <div>
            <h2 className="text-xl font-bold text-orange-400 mb-2">User Orders</h2>
            {user.orders?.length ? (
              <ul className="space-y-2">
                {user.orders.map((order, idx) => (
                  <li
                    key={idx}
                    className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg text-sm font-mono overflow-x-auto flex justify-between items-center"
                  >
                    <span className="flex-1 pr-2">
                      {typeof order === 'object' ? JSON.stringify(order, null, 2) : order}
                    </span>
                    <Button variant="ghost" size="icon" onClick={() => deleteOrder(idx)}>
          ...
          {/* Reviews */}
          <div>
            <h2 className="text-xl font-bold text-orange-400 mb-2">User Reviews</h2>
            {user.reviews?.length ? (
              <ul className="space-y-2">
